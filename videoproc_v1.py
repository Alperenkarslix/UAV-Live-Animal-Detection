...
import random
import json
from video_common import auto_orient, read_with_loop
...
class VideoProcessor:
    def __init__(self, video_file, target_width, target_height, num_animals):
        ...
        self.animals = self.create_animals(num_animals, data['animal_coords'])

    def create_animals(self, num_animals, animal_coords):
        keys = list(animal_coords.keys())
        if not keys:
            return []

        pick_count = min(num_animals, len(keys))
        chosen_keys = random.sample(keys, pick_count)
        ...
        for i, key in enumerate(chosen_keys):
            ...
            animal = {
                ...
                'color': (0, 255, 0),
                'coordinates': animal_coords[key],
            }
            animals.append(animal)
            print(f'{animal_name} coordinates: {animal_coords[key]}')
        return animals
    ...
    def process_video(self):
        ...
        while self.cap.isOpened():
            ret, frame = read_with_loop(self.cap)
            if not ret:
                break
            ...
            if original_width > self.target_width or original_height > self.target_height:
                ...
                original_height, original_width = new_h, new_w
            ...
            display = background
            if self.target_width > 1280 or self.target_height > 720:
                scale = min(1280 / self.target_width, 720 / self.target_height)
                display = cv2.resize(background, (int(self.target_width * scale), int(self.target_height * scale)), interpolation=cv2.INTER_AREA)
